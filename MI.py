...
if not os.path.exists(path):
  os.makedirs(path)
num_baseline = 20 # number of baseline correlation by random shuffling each sequence
num_sample = 20 # number of random sampling
for file in files:
  if file.endswith(".npz"):
    ...
    for b in range(num_baseline):
      ...
      adj_mat_bl[:, :, b] = corr_mat(sample_seq, measure)
