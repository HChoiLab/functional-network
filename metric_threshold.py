...
for ind, threshold in enumerate(thresholds):
  ...
  for row_ind, row in enumerate(rows):
    for col_ind, col in enumerate(cols):
      G = G_dict[row][col]
      densities[row_ind, col_ind, ind] = nx.density(G)
      largest_cc = max(nx.connected_components(G), key=len)
      G = nx.subgraph(G, largest_cc)
      num_nodes[row_ind, col_ind, ind] = G.number_of_nodes()
      num_edges[row_ind, col_ind, ind] = G.number_of_edges()
  cc = True
...
# save data
print('saving data...')
# stat_dict.update({'delta metrics':delta_metrics})
a_file = open('./data/ecephys_cache_dir/sessions/stat_dict.pkl', 'wb')
pickle.dump(stat_dict, a_file)
a_file.close()
print("--- %s minutes ---" % ((time.time() - start_time)/60))

# %%
# plot stats
# a_file = open('./data/ecephys_cache_dir/sessions/stat_dict.pkl', 'rb')
...
# keys.remove('metrics')
# keys.remove('delta metrics')
# fig = plt.figure(figsize=(5*len(keys), 4))
# left, width = .25, .5
# bottom, height = .25, .5
# right = left + width
# top = bottom + height
# ind = 1
# for key in keys:
#   plt.subplot(1, len(keys), ind)
#   ind += 1
#   plt.gca().set_title(key, fontsize=20, rotation=0)
#   # plt.tick_params(left=True, bottom=True, labelleft=True, labelbottom=True)
#   sns_plot = sns.heatmap(stat_dict[key].astype(float), cmap="YlGnBu", norm=LogNorm())
#   # sns_plot = sns.heatmap(region_connection.astype(float), vmin=0, cmap="YlGnBu")
#   sns_plot.set_xticks(np.arange(len(thresholds))+0.5)
#   sns_plot.set_xticklabels(thresholds, rotation=0)
#   # sns_plot.set_yticks(np.arange(stat_dict[key].shape[0])+0.5)
#   # sns_plot.set_yticklabels(range(1, stat_dict[key].shape[0] + 1), rotation=0)
# plt.tight_layout()
# plt.savefig('./plots/stats_threshold_{}.jpg'.format(measure))
# # shapes = delta_metrics.shape
# # delta_metrics = delta_metrics.reshape(shapes[0] * shapes[1], shapes[2], shapes[3])
...
#   plt.title(metric_name, fontsize=20, rotation=0)
#   # plt.tick_params(left=True, bottom=True, labelleft=True, labelbottom=True)
#   sns_plot = sns.heatmap(flat(stat_dict['metrics'][:, :, metric_ind, :]).astype(float), cmap="YlGnBu")
#   # sns_plot = sns.heatmap(region_connection.astype(float), vmin=0, cmap="YlGnBu")
#   sns_plot.set_xticks(np.arange(len(thresholds))+0.5)
#   sns_plot.set_xticklabels(thresholds, rotation=0)
#   # sns_plot.set_yticks(np.arange(stat_dict[key].shape[0])+0.5)
#   # sns_plot.set_yticklabels(range(1, stat_dict[key].shape[0] + 1), rotation=0)
# plt.tight_layout()
# plt.savefig('./plots/metrics_threshold_{}.jpg'.format(measure))
...
#   print(metric_name)
#   plt.subplot(2, 4, metric_ind + 1)
#   plt.title(r'$\Delta$'+metric_name, fontsize=20, rotation=0)
#   # plt.tick_params(left=True, bottom=True, labelleft=True, labelbottom=True)
#   sns_plot = sns.heatmap(flat(stat_dict['delta metrics'][:, :, metric_ind, :]).astype(float), cmap="YlGnBu")
#   # sns_plot = sns.heatmap(region_connection.astype(float), vmin=0, cmap="YlGnBu")
#   sns_plot.set_xticks(np.arange(len(thresholds))+0.5)
#   sns_plot.set_xticklabels(thresholds, rotation=0)
#   # sns_plot.set_yticks(np.arange(stat_dict[key].shape[0])+0.5)
